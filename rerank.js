...
const FAVICON = 'data:image/svg+xml,' + encodeURIComponent(
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32"><rect width="32" height="32" rx="7" fill="#f6f6ef"/><path d="M16 7 L26 24 L6 24 Z" fill="#ff6600"/></svg>')
...
function renderPage(stories, stats, hero, rescues) {
  ...
  *{box-sizing:border-box}html{scroll-behavior:smooth}
  ...
  h1.brand{font-size:30px;font-weight:800;letter-spacing:-.02em;margin:0;color:var(--ink)}
  .brand .tri{color:var(--hn)}
  ...
  .deck{font-size:17px;color:var(--body);margin:12px 0 0;max-width:580px;line-height:1.55}
  ...
  .stitle h3 a{color:var(--ink);text-decoration:none}
  ...
  .lane{margin-top:8px}
  .featurednote{font-family:var(--mono);font-size:12.5px;color:var(--hn-deep);margin:8px 0 0}
  .comment{padding:12px 0;border-top:1px dashed rgba(0,0,0,.12)}
  ...
  .ctext pre{background:rgba(0,0,0,.05);border:1px solid rgba(0,0,0,.08);color:#23231d;padding:9px 11px;border-radius:12px;overflow:auto;font-size:13px;font-family:var(--mono)}
  ...
  details.more summary:hover{color:var(--hn-dark)}
...
function renderOgSvg(hero, stats) {
  ...
  const quoteLines = lines.map((l, i) =>
    `<text x="120" y="${330 + i * 46}" font-family="Helvetica, Arial, sans-serif" font-size="30" fill="#3d3d3a">${escapeXml(l)}</text>`).join('\n')
  return `<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630">
  <defs>
    <radialGradient id="g1" cx="15%" cy="20%" r="55%"><stop offset="0%" stop-color="#FF6600" stop-opacity=".9"/><stop offset="100%" stop-color="#FF6600" stop-opacity="0"/></radialGradient>
    <radialGradient id="g2" cx="88%" cy="12%" r="50%"><stop offset="0%" stop-color="#FFB000" stop-opacity=".8"/><stop offset="100%" stop-color="#FFB000" stop-opacity="0"/></radialGradient>
    <radialGradient id="g3" cx="78%" cy="85%" r="55%"><stop offset="0%" stop-color="#FF7B2E" stop-opacity=".8"/><stop offset="100%" stop-color="#FF7B2E" stop-opacity="0"/></radialGradient>
    <radialGradient id="g4" cx="25%" cy="88%" r="50%"><stop offset="0%" stop-color="#FFCC70" stop-opacity=".7"/><stop offset="100%" stop-color="#FFCC70" stop-opacity="0"/></radialGradient>
  </defs>
  <rect width="1200" height="630" fill="#f6f6ef"/>
  ...
  <rect x="60" y="80" width="1080" height="470" rx="36" fill="#FFFFFF" fill-opacity="0.55" stroke="#FFFFFF" stroke-opacity="0.9" stroke-width="1.5"/>
  <text x="120" y="160" font-family="Menlo, monospace" font-size="21" letter-spacing="4" fill="#6b6b60">SLOW RANK · TODAY'S RESCUE</text>
  <text x="120" y="245" font-family="Helvetica, Arial, sans-serif" font-weight="bold" font-size="52" fill="#f25c00">${escapeXml(headline)}</text>
  ${quoteLines}
  <text x="120" y="${330 + lines.length * 46 + 28}" font-family="Menlo, monospace" font-size="21" fill="#6b6b60">${escapeXml(attrib)}</text>
  <text x="120" y="600" font-family="Menlo, monospace" font-size="17" fill="#6b6b60">${escapeXml(`${fmt(stats.comments || 0)} comments read this hour · scasella.github.io/hn-slow-rank`)}</text>
  </svg>`
}
